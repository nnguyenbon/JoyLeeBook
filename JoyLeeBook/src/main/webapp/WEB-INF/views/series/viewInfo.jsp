                    ...
                    <c:when test="${sessionScope.loggedInUser != null and sessionScope.loggedInUser.roleName eq 'admin'}">
                        <div class="d-flex gap-2 my-5">
                            <a class="btn btn-warning"
                               href="${pageContext.request.contextPath}/updateSeries?seriesId=${series.seriesId}"><i
                                    class="bi bi-pen"></i> Edit</a>
                            <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal"
                                    data-bs-target="#deleteModal"
                                    data-id="${series.seriesId}" data-type="series" data-url="deleteSeries"><i
                        ...
                        <div class="d-flex gap-2 my-5">
                            <c:if test="${not empty listChapter}">
                                <a class="btn btn-primary text-white"
                                   href="${pageContext.request.contextPath}/readChapter?chapterIndex=${listChapter[0].chapterIndex}&seriesId=${series.seriesId}"><i
                                        class="bi bi-book"></i> Start Reading</a>
                            </c:if>
                            <c:if test="${sessionScope.role != null and sessionScope.role eq 'user'}">
                                <a class="btn btn-outline-primary"
                                   href="${pageContext.request.contextPath}/saveSeries?seriesId=${series.seriesId}"><i
                                        class="bi bi-bookmark"></i> Add Library</a>
                            </c:if>
                        </div>
                    </c:otherwise>
                </c:choose>
        ...
        <div class="mt-5 border border-2 rounded-4 p-4 m-0 g-4" style="background-color: #ffffff">
            <div class="d-flex justify-content-between align-items-center mb-4 mx-2">
                ...
                <c:if test="${sessionScope.loggedInUser != null and sessionScope.loggedInUser.roleName eq 'admin'}">
                    <a href="${pageContext.request.contextPath}/addChapter?seriesId=${series.seriesId}"
            ...
            <div class="container my-4 py-4" style="max-height: 500px; overflow-y: auto">
                ...
                <c:forEach var="chapter" items="${listChapter}">
                    <div class="chapter-row d-flex justify-content-between align-items-center px-5 py-2 mb-3 rounded-3 border position-relative"
                         onclick="window.location = '${pageContext.request.contextPath}/readChapter?chapterIndex=${chapter.chapterIndex}&seriesId=${chapter.seriesId}'"
                        ...
                        <div class="d-flex align-items-center gap-5">
                            ...
                            <div>
                                ...
                                <div class="text-muted small mt-1"><fmt:formatDate value="${chapter.createdAt}"
                        ...
                        <c:choose>
                            <c:when test="${sessionScope.loggedInUser != null and sessionScope.loggedInUser.roleName eq 'admin'}">
                                <div class="action-buttons gap-2">
                                    <a href="${pageContext.request.contextPath}/updateChapter?chapterId=${chapter.chapterId}"
                                       class="btn btn-sm btn-outline-warning" onclick="event.stopPropagation()"><i
                                            class="bi bi-pen"></i></a>
                                    <button type="button" class="btn btn-sm btn-outline-danger" data-bs-toggle="modal"
                                            data-bs-target="#deleteModal"
                                            data-id="${chapter.chapterId}" data-series-id="${chapter.seriesId}"
                                            data-type="chapter" data-url="deleteChapter"
